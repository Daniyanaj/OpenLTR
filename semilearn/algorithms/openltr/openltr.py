...
from semilearn.algorithms.utils import CE_weight, ce_loss, consistency_loss, consistency_open_loss, SSL_Argument, str2bool , CB_loss
from semilearn.algorithms.utils.loss import consistency_open_loss
...
class OpenNet(nn.Module):
    ...
    def forward(self, x, y_lb, **kwargs):
        feat = self.backbone(x, y_lb, only_feat=True)
        feat_c=self.backbone(feat, y_lb, only_featc=True)
        logits_etf= self.backbone(feat_c,y_lb, only_etf=True)
        logits = self.backbone(feat, y_lb, only_fc=True)
        ...
        return_dict = {'feat': feat, 'feat_proj': feat_proj, 'feat_c':feat_c,
                       'logits': logits, 'logits_open': logits_open, 'logits_mb': logits_mb, 'logits_etf' : logits_etf}
        if self.use_rot:
...
class OpenLTR(AlgorithmBase):
    ...
    def set_model(self):
        ...
        model = OpenNet(model, num_classes=self.num_classes, use_rot=self.args.use_rot)
        return model

    def set_ema_model(self):
        ema_model = self.net_builder(num_classes=self.num_classes)
        ema_model = OpenNet(ema_model, num_classes=self.num_classes, use_rot=self.args.use_rot)
    ...
    def train_step(self, x_lb, y_lb, x_ulb_w, x_ulb_s):
        ...
        with self.amp_cm():
            if self.use_cat:
                ...
                outputs = self.model(inputs, y_lb)
                feats_c= outputs['feat'][:num_lb]
                logits_etf= outputs['logits_etf'][:num_lb]
            ...
            y_c = torch.arange(0, self.num_classes).cuda(self.gpu)

            cr= ce_loss(logits_etf, y_c, reduction='mean')

            
            sup_closed_loss1= ce_loss(logits_x_lb, y_lb, reduction='mean') 
            sup_closed_loss = 1.25*sup_closed_loss1 + 0.75* cr
            ...
            #ui_loss = consistency_loss(logits_x_ulb_s,  targets_p,self.f_score_list, cls_num_list, 'ce',E=self.epoch, mask=in_mask * p_mask)
            ui_loss = consistency_open_loss(logits_x_ulb_s,  targets_p, 'ce', mask=in_mask * p_mask)
            op_loss = consistency_open_loss(logits_open_x_ulb_s, targets_q, 'ce', mask=q_mask)
            #op_loss = consistency_loss(logits_open_x_ulb_s, targets_q,self.f_score_list,  cls_num_list, 'ce',E=self.epoch, mask=q_mask)
