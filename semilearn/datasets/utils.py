...
def sample_labeled_data(args, data, target, num_labels, num_classes, index=None):
    ...
    dump_path= '/data1/daniya_miccai24/NC/OpenLTR/data/isic2018_openset/labeled_idx/25p.npy'
    ...
    #     samples_per_class=int(0.75*len(idx))
    ...
    def load_labeled_data(dump_path):
        # Load the labeled indices from the .npy file
        labeled_indices = np.load(dump_path)
        
        # Use the loaded indices to retrieve labeled data and labels from your dataset
        # Example: 
        # lb_data = data[labeled_indices]
        # lbs = target[labeled_indices]
        
        return  labeled_indices

# Example usage
    # print("lll",load_labeled_data(dump_path))
    idx = np.load(dump_path, allow_pickle=True)
    lb_idx.extend(idx)

    lb_data.extend(data[idx])
    lbs.extend(target[idx])

    # print("aaaaaaaa",len(data[idx]))
#     #print("aaaaaa",len(lbsa))
    print('len',len(lb_idx))
...
def sample_unlabeled_data(args, data, target, num_labels, num_classes, index=None):
    '''
    samples for labeled data
    (sampling with balanced ratio over classes)
    '''
...
def split_ossl_data(args, data, target, num_labels, num_all_classes, seen_classes, index=None, include_lb_to_ulb=True):
    ...
    target_f=[]
    data_f=[]
    for label in range(7):  # Labels 0 to 4
        indices = np.where(target== label)[0]
        data_f.extend(data[indices].tolist())
        target_f.extend(target[indices].tolist())

    # # For labels 5 and 6, fetch only 100 samples each
    # for label in range(5, 7):
    #     indices = np.where(target == label)[0][:100]  # Get the first 100 indices
    #     data_f.extend(data[indices].tolist())
    #     target_f.extend(target[indices].tolist())
    #######    
    ...
    if include_lb_to_ulb:
        return lb_data, lbs, data_f, target_f
    else:
