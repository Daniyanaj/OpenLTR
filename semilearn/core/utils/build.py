...
def get_dataset(args, algorithm, dataset, num_labels, num_classes, data_dir='./data',
    ...
    from semilearn.datasets.cv_datasets.isic2018 import get_isic2018_openset
    ...
    if dataset in ["cifar10_openset", "cifar100_openset"]:
        ...
        lb_dset, ulb_dset, eval_dset, eval_full_dset = get_imagenet30(args, algorithm, dataset, args.labeled_percent,
                                                                      num_classes=7, data_dir=data_dir)
        if eval_open:
            test_dset = {'full': eval_full_dset}
    elif dataset== 'isic2018_openset':
        # lb_dset, ulb_dset, eval_dset, eval_full_dset = get_isic2018_openset(args, algorithm, dataset, args.labeled_percent,
        #                                                               num_classes, data_dir=data_dir)
        lb_dset, ulb_dset, eval_dset, eval_full_dset = get_isic2018_openset(args, algorithm, dataset,
                                                                       num_classes=num_classes, num_labels=num_labels, data_dir=data_dir)
        if eval_open:
            test_dset = {'full': eval_full_dset}
