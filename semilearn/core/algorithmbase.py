...
class AlgorithmBase:
    ...
    def train(self):
        ...
        for epoch in range(self.epoch, self.epochs):
            ...
            if self.it >= self.num_train_iter:
                print("ssssssssssssssssssssssssssssssssssssss",self.it)
                break
    ...
    def evaluate(self, eval_dest='eval', return_logits=False):
        ...
        total_loss = 0.0
        total_num = 0.0
        f_score_list= [1.0 for _ in range(self.num_classes)]
        val_confusion_diag = ConfusionMatrix(num_classes=self.num_classes, labels=list(range(self.num_classes)))
        y_true = []
        y_pred = []
        # y_probs = []
        y_logits = []
        ...
        if self.num_classes <= 10:
            ...
            val_confusion_diag.update(y_pred, y_true)
            dia_acc = val_confusion_diag.summary()
            f_score_list=val_confusion_diag.get_f1score()
            #print(val_confusion_diag)

            #self.print_fn('confusion matrix:\n' + np.array_str(val_confusion_diag))
            #self.print_fn('f_score:\n' + np.array_str(f_score_list))
            print(f_score_list)
            #print(dia_acc)
        self.f_score_list=f_score_list    
        self.ema.restore()
        self.model.train()
    ...
    def get_save_dict(self):
        ...
        save_dict = {
            ...
            #'f_score_list':self.f_score_list
        }
        return save_dict
